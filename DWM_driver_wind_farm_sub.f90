...
!...................................................................
    ...
    USE read_wind_farm_parameter_data
    USE DWM_init_data,                 ONLY: FAST8
    IMPLICIT NONE
    
    IF (FAST8 == 1) THEN
        OPEN(unit = 10, status='replace',file='DWM-driver\DWM_parameter.bin',form='unformatted')    
        WRITE(10)   HubHt,RotorR,NumWT,Uambient,TI,Domain_R,Domain_X,ppR,Mstl,Mmt,WFLowerBd,Winddir,Tinfluencer                                                                                                                                                                                                   
        CLOSE(10)
    ELSE
        OPEN(unit = 10, status='replace',file='DWM-driver\DWM_parameter.bin',form='unformatted')    
        WRITE(10)   HubHt,HubHt,NumWT,Uambient,TI,Domain_R,Domain_X,ppR,Mstl,Mmt,WFLowerBd,Winddir,Tinfluencer                                                                                                                                                                                                   
        CLOSE(10)
    END IF
      
    OPEN(unit = 10, status='replace',file='DWM-driver\wind_farm_coordinate.bin',form='unformatted')    
    WRITE(10)   Xcoordinate,Ycoordinate                                                                                                                                                                                                    
    CLOSE(10)
...
!...................................................................
    ...
    OPEN(unit = 10, status='replace',file='DWM-results\wind_farm_turbine_sort.bin',form='unformatted')    
    WRITE(10)   turbine_sort(:)                                                                                                                                                                                                    
    CLOSE(10)
    ...
    OPEN(unit = 10, status='replace',file='DWM-results\turbine_angles.bin',form='unformatted')    
    WRITE(10)   turbine_angle(:,:)                                                                                                                                                                                                    
    CLOSE(10)
    
    OPEN(unit = 10, status='replace',file='DWM-results\turbine_distance.bin',form='unformatted')    
    WRITE(10)   length(:)                                                                                                                                                                                                    
    CLOSE(10)
...
!............................................................................
    ...
    IF ( SimulationOrder_index > 0 ) THEN            ! exclude the first turbine
        ...
        RESULT =rename( TRIM(OutFileRoot)//'.elm',filename_FastElm   )
    END IF

END SUBROUTINE rename_FAST_output

!----------------------------------------------------------------------------------
SUBROUTINE HH_patern()
!............................................................................
! This routine is called to specify the HH of each turbine
!............................................................................
    USE   wind_farm_geometry_data,        ONLY: turbine_sort, HH_array
    USE   read_wind_farm_parameter_data,  ONLY: HubHt, NumWT
    
    INTEGER    ::    I
    
    ALLOCATE ( HH_array(NumWT) )
    ...
    HH_array = (/ 70,110,70,110,70/)
    
    
END SUBROUTINE HH_patern

!-------------------------------------------------------------------
SUBROUTINE write_HH_to_file(simulation_index)
!...................................................................
! This subroutine is to write the turbine spacing from the input file
! to a bin file, which enables the DWM to read the turbine spacing at every
! instances of running
!...................................................................
    ...
    USE read_wind_farm_parameter_data
    USE wind_farm_geometry_data,        ONLY: HH_array

    INTEGER   ::    simulation_index
    
    IF (simulation_index < NumWT) THEN
        OPEN(unit = 10, status='replace',file='DWM-driver\DWM_parameter.bin',form='unformatted')    
        WRITE(10)   HH_array(simulation_index), HH_array( simulation_index+1 ), NumWT,Uambient,TI,Domain_R,Domain_X,ppR,Mstl,Mmt,WFLowerBd,Winddir,Tinfluencer                                                                                                                                                                                                   
        CLOSE(10)
    ELSE
        OPEN(unit = 10, status='replace',file='DWM-driver\DWM_parameter.bin',form='unformatted')    
        WRITE(10)   HH_array(simulation_index), HH_array( simulation_index   ), NumWT,Uambient,TI,Domain_R,Domain_X,ppR,Mstl,Mmt,WFLowerBd,Winddir,Tinfluencer                                                                                                                                                                                                   
        CLOSE(10)
    END IF
    
    
END SUBROUTINE write_HH_to_file
